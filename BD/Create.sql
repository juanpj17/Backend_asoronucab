...
CREATE OR REPLACE FUNCTION public.seleccionar_empleados()
    RETURNS TABLE(codigo integer, cedula character varying, rif character varying, p_nombre character varying, s_nombre character varying, p_apellido character varying, s_apellido character varying, direccion character varying, sueldo numeric, fecha_ing timestamp without time zone, parroquia integer) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE PARALLEL UNSAFE
    ROWS 1000
AS $BODY$
begin
return query SELECT per_nat_id,per_nat_ci, per_nat_rif, per_nat_p_nombre, per_nat_s_nombre, per_nat_p_apellido, per_nat_s_apellido, per_nat_direccion, emp_sueldo, emp_fecha_ingreso, fk_lugar
	FROM public."Empleado";
end;
$BODY$;
ALTER FUNCTION public.seleccionar_empleados()
    OWNER TO postgres;
